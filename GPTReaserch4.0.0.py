...
class ResearchBot:
    ...
    def scrape_webpage(self, url):
        ...
        try:
            ...
            # Summarize the content using llama3.1
            ...
            Keep only factual, relevant information that directly relates to the topic:
    ...
    def create_claude_prompt(self, topic, format_type):
        """Create a prompt for Claude with all available information"""
        print(f"\n{Fore.CYAN}Creating prompt for Claude...{Style.RESET_ALL}")
    ...
    - Cites at least 8-10 different sources
    ...
    --readlink-- [number of link to read]
    ...
    def process_claude_response(self, response):
        """Process Claude's response and execute requested actions"""
        print(f"\n{Fore.YELLOW}Claude's response:{Style.RESET_ALL}")
        
        # Handle the TextBlock format
        if isinstance(response, list):
            response_text = response[0].text if response else ""
        else:
            response_text = str(response)  # Convert to string if not already
        ...
        if '--websearch--' in response_text:
            ...
            # Add search results to data file
            if search_results:
    ...
    def research(self, topic, format_type):
        ...
        while continue_research and iteration < 15:
            ...
            prompt = self.create_claude_prompt(topic, format_type)
            
            try:
                print(f"{Fore.CYAN}Sending request to Claude...{Style.RESET_ALL}")
                response = self.client.chat.completions.create(
                    model="deepseek-chat",
                    messages=[{"role": "user", "content": prompt}]
                )
                
                continue_research = self.process_claude_response(response.choices[0].message.content)
                self.log_history(f"Iteration {iteration}: {response.choices[0].message.content}")
                
            except Exception as e:
                print(f"{Fore.RED}Claude API error: {str(e)}{Style.RESET_ALL}")
                self.log_history(f"Claude API error: {str(e)}")
